...
#   1. Transform each `set` in CMakeCUDACompiler to be a `PARENT_SCOPE`
#      This allows us to propagate up immediate information that is
#      used by commands such target_compile_features.
#
#   2. Include `CMakeCUDAInformation` this can't be deferred as the contents
#      are required if any target is constructed
#
# For root directories we only need to include `CMakeCUDAInformation`
#

# Expose the language at the current scope
enable_language(CUDA)

if(NOT EXISTS "${CMAKE_BINARY_DIR}/cmake/PropagateCMakeCUDACompiler.cmake")
  # 1.
  # Take everything that `enable_language` generated and transform all sets to PARENT_SCOPE ()
  # This will allow our parent directory to be able to call CMakeCUDAInformation
  file(STRINGS "${CMAKE_BINARY_DIR}/CMakeFiles/${CMAKE_VERSION}/CMakeCUDACompiler.cmake" rapids_code_to_transform)
  ...
  # 2.
  # Make sure we call "CMakeCUDAInformation" for the current directory
  string(APPEND rapids_code_to_execute "include(CMakeCUDAInformation)\n")

  file(WRITE "${CMAKE_BINARY_DIR}/cmake/PropagateCMakeCUDACompiler.cmake" "${rapids_code_to_execute}")
  unset(rapids_code_to_execute)
  unset(rapids_code_to_transform)
endif()

# propagate up one parent_scope
include("${CMAKE_BINARY_DIR}/cmake/PropagateCMakeCUDACompiler.cmake")

# Compute all directories between here and the root of the project
# - Each directory but the root needs to include `PropagateCMakeCUDACompiler.cmake`
# - Since the root directory doesn't have a parent it only needs to include
#   `CMakeCUDAInformation`
...
foreach(rapids_directory IN LISTS rapids_directories)
  ...
  if(NOT rapids_CUDA_hook IN_LIST rapids_existing_calls)
    cmake_language(DEFER DIRECTORY "${rapids_directory}"
                   ID rapids_CUDA_hook
                   CALL include "${CMAKE_BINARY_DIR}/cmake/PropagateCMakeCUDACompiler.cmake")
  else()
    break()
  endif()
endforeach()

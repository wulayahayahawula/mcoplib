...
public:
  ...
  void run(const std::vector<std::string> &argv)
  {
    ...
    try
    {
      // Initialize CUDA and benchmark_manager before running benchmarks
      // This ensures CUDA is initialized even if it failed during import
      try {
        NVBENCH_DRIVER_API_CALL(cuInit(0));
      } catch (const std::exception& cuda_e) {
        throw std::runtime_error("CUDA initialization failed before benchmark execution: " + std::string(cuda_e.what()));
      }
      ...
      } catch (const std::exception& manager_e) {
        throw std::runtime_error("Benchmark manager initialization failed: " + std::string(manager_e.what()));
      }
...
PYBIND11_MODULE(_nvbench, m)
{
  ...
  // Note: Delay CUDA initialization to avoid import-time errors

  static bool cuda_initialized = false;

  // Initialize CUDA with error handling for version compatibility
  try {
    NVBENCH_DRIVER_API_CALL(cuInit(0));
    cuda_initialized = true;
  } catch (const std::exception& e) {
    // Don't fail at import time - let the user know but continue
    std::fprintf(stderr, "Warning: CUDA initialization failed at import time: %s\n", e.what());
    std::fprintf(stderr, "         CUDA functions will be initialized on first use\n");
    cuda_initialized = false;
  }

  // Initialize benchmark_manager only if CUDA is available
  if (cuda_initialized) {
    try {
      nvbench::benchmark_manager::get().initialize();
    } catch (const std::exception& e) {
      std::fprintf(stderr, "Warning: Benchmark manager initialization failed: %s\n", e.what());
      std::fprintf(stderr, "         Manager will be initialized on first benchmark run\n");
    }
  }
